...
(async () => {
    ...
    try {
        ...
        await page.locator('[data-heading-alignment="center"]').click();
        await page.locator('[data-body-alignment="right"]').click();
        await page.locator('#heading-bold').check();
        ...
        await page.locator('#field-type').selectOption('image');
        await page.locator('#field-label').fill('実験写真');
        await page.locator('#image-size').selectOption('large');
        await page.locator('[data-image-alignment="right"]').click();
        await page.locator('#field-apply').click();
        assert.equal(await page.locator('#preview-paper .preview-image-placeholder').count(), 3);
        await page.locator('#save-template').click();
        await page.getByText('保存エラー', {exact: true}).waitFor();
        assert.equal(await page.locator('#field-list > li').count(), 3);
        assert.equal(await page.locator('#template-name').inputValue(), '確認テンプレート');
        assert.equal(payload.fields[0].heading_font_size, 24);
        assert.equal(payload.fields[0].heading_alignment, 'center');
        assert.equal(payload.fields[0].body_alignment, 'right');
        assert.equal(payload.fields[0].heading_bold, true);
        assert.equal(payload.fields[0].body_bold, false);
        assert.equal(payload.fields[1].heading_alignment, 'left');
        assert.equal(payload.fields[1].heading_bold, false);
        assert.equal(await page.locator('#preview-paper h3').first().evaluate(el => el.style.textAlign), 'center');
        assert.equal(await page.locator('#preview-paper h3').first().evaluate(el => el.style.fontWeight), '700');
        assert.equal(payload.fields[1].hide_heading, true);
        assert.equal(payload.fields[2].field_type, 'image');
        assert.equal(payload.fields[2].image_size, 'large');
        assert.equal(payload.fields[2].image_alignment, 'right');
        assert.equal(payload.fields[2].max_length, 50);
